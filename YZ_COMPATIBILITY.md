...
- 源码版本升至 `1.13.4`；尚未创建对应 Tag、Release 或镜像，当前正式发布来源仍为下节的 `1.13.3`。
...
- Node 通信和两个核心依赖保持原配套关系，本次没有新增 Node 或核心版本要求。
- 本地验证：管理端 19 项测试通过，其中新增 8 项覆盖整数与小数填价、重复输入、空值与零值、已有价格恢复、完整补丁重复执行及锚点异常；PHP 补丁与版本配置、生成的 JavaScript 均通过语法检查。镜像构建和发布尚未执行。

## 上游同步与功能合并发布（1.13.3，已发布）

| 项目 | 标识 |
| --- | --- |
| 当前正式面板版本 | `1.13.3`；统一包含上游同步、默认内核和 64 MiB 插件上传，Tag、Release 与双架构镜像已发布 |
| 正式来源 Tag / commit | `v1.13.3` / `f88d243b7e7683e183bf0a3d62af1173220b0855`；后续发布记录提交不改变此构建来源 |
| 正式 Release | [v1.13.3](https://github.com/P0me1oo/YZboard/releases/tag/v1.13.3)，最新正式版本；2026-09-10 发布并核验 |
...
服务器更新由用户执行。面板回滚使用本节记录的 `1.13.0-2f29916` 镜像，Node 回滚使用 `v1.13-yz.24`；更新前保留原 Compose、旧镜像和必要的数据备份。以下各节保留此前的开发和历史发布记录，其中“待发布”等状态仅对应当时的验证阶段，当前发布引用以本节为准。
