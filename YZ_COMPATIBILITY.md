...
- 配套 Node `v1.21.0`、管理端工程 `0.8.0`，修改起点为已发布的面板 `1.24.0`、Node `v1.20.0`、管理端 `0.7.0`。
- 面板的设备计数、设备详情和下发给 Node 的设备快照统一把公网 IPv6 规范化为所在 `/64` 网段；IPv4 继续按单个公网地址计数，名单过滤先于网段合并。
- 面板接收 Node 的设备快照时保存完整公网来源，在读取设备数和下发跨节点快照时才按当前名单排除并合并 IPv6 `/64`。精确排除单个 IPv6 地址不再误伤同网段其他地址；旧 Node 上报的已合并网段无法还原完整地址，待升级节点并刷新快照后生效。
- 已处理的流量报告超过七天只清空大字段，永久保留批次编号和唯一约束。节点长期离线后重发同一批次仍不会重复结算；数据库会持续保存精简编号记录，应按实际报告量监测容量。
- 系统设置的 `device_ip_exclude` 仍使用原字段和保存校验，管理提示改为前置服务器 IP，覆盖转发机和链式代理前置服务器。没有数据库迁移。
- 管理端 `0.8.0` 构建产物已同步到 `public/assets/admin`，面板侧 5 项资源检查通过。PHP 完整回归通过（PHP 8.4.21，313 项测试、3,697 个断言；2026-09-26），未连接真实服务器。面板提交 `226dee79f3a2029590ab69b9e143f708562e1a73` 的 [Release v1.25.0](https://github.com/P0me1oo/YZboard/releases/tag/v1.25.0) 已发布；镜像 `ghcr.io/p0me1oo/yzboard:1.25.0-226dee7`、`1.25.0` 和 `latest` 均为 `sha256:c8d60ab6f39cfafb66fe97e4b5ebedafa05e776b0d3273f6d5c415f46b7b3d8c`，包含 `linux/amd64`（`sha256:3dd19882195375a3ae11f55c9c482c8add012d819936f17bd55db6159a0a7bd6`）和 `linux/arm64`（`sha256:a842f71ef48f6a9d484eb26a2c706e9ebade73a46919a4867b9c592f2ff09d2a`）；两个架构的 OCI 来源提交和版本标签已核对。面板镜像工作流的 PHP 回归、管理端资源检查、构建和清单核验均通过。
- 回滚基线为面板 `1.24.0`、Node `v1.20.0` 和管理端 `0.7.0`。
