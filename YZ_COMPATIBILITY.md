...
- 配套 Node `v1.17.3` / 提交 `80e993b2c843d6642946a081b4c5f4f1d16450a8`，管理端工程 `0.4.7` / 本地源码提交 `3a970f38f77b68aea43013b748354e48a3f17b82`。面板和 Node 均尚未发布；旧 Node 会继续将内网地址用于本地设备准入，因此需先升级 Node，再更新面板。
- 本地验证：面板完整 PHP 回归 294 项、3581 个断言通过；管理端源码检查、19 项行为测试、构建检查、原有 23 项浏览器测试和新增的桌面/触屏 2 项浏览器测试通过，面板产物契约 5 项通过；构建产物已同步到面板。PHP 测试通过命令行临时加载本机已有 SQLite 扩展完成，真实后端联调尚未进行。
