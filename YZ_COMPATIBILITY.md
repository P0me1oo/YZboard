...
## 上游同步与功能合并发布（1.13.3，发布准备）

| 项目 | 标识 |
| --- | --- |
| 目标面板版本 | `1.13.3`；统一包含上游同步、默认内核和 64 MiB 插件上传，采用固定 Tag 发布 |
...
| 配套关系 | 先发布 Node `v1.13.1`，再发布面板 `v1.13.3`；没有新增数据库迁移，也未改变 Node 通信字段或固定核心依赖 |
...
两处测试初始化冲突保留 YZboard 的内存数据库、缓存隔离和节点通信扩展测试；没有删除上游新增的订单测试。订单相关用例验证旧模型、重复调用、退款失败重试及礼品卡事务回滚，本地同步阶段未执行 MySQL 多连接并发测试；正式镜像构建和校验由发布工作流完成。

原有默认内核和 64 MiB 插件上传修改一并纳入 `v1.13.3` 的发布提交；此前 `1.13.1`、`1.13.2` 为未发布的开发计划，不另行创建 Tag。配套 Node 使用标准正式版本 `v1.13.1`，历史版本保留原名用于升级和回滚。

正式发布前，工作流先在 PHP 8.2 运行完整面板和管理端补丁测试，再构建 `linux/amd64`、`linux/arm64` 镜像。发布后补充本版固定提交、镜像标签与摘要，并核对版本别名和 `latest`；服务器更新由用户执行。下方保留此前的开发和发布记录。
...
发布时先提供 Node `v1.13.1` 的安装器及 xbctl，再发布面板 `1.13.3`。Node 的旧配置加载规则仍将省略内核的历史配置解释为 Xray，具体说明见 Node 的兼容矩阵。
