...
## 权限组排序与名称显示（1.18.0，未发布）

| 项目 | 标识 |
| --- | --- |
| 当前状态 | 未发布。已提交到 `master`，排在本节下方的连接数限制之后；无 Tag、Release 和镜像，`latest` 仍指向 `1.16.0` |
...
| 管理前端 | YZboard-Dash `0.3.0`：权限组页排序模式与名称完整显示；源码检查 3003 文件、12 项单元测试、开发模式冒烟通过（含长组名不截断、拖拽提交内容、保存后列表顺序） |
| 未在本地验证 | 管理端产物未重新构建，`public/assets/admin` 仍是合并前的构建；构建检查、浏览器测试和镜像构建未执行 |
| 发布前提 | 重新构建并同步管理端产物，补跑构建检查与浏览器测试；`/server/group` 的页面对照已增加“编辑排序”按钮豁免，该豁免尚未实际执行过 |
...
| 管理端产物 | 产物契约 5 项测试通过，八组 YZ 定制均在新产物内 |
| 未在本地验证 | 未做面板与真实 Node 的端到端联调；本机无 Docker，镜像构建由发布工作流完成 |
| 前端验证缺口 | 管理端新增表单的浏览器逐字对照未完成。YZboard-Dash 的页面对照测试按固定顺序比对 29 个路由，当前在 `/server/group` 因另一项在途改动（权限组排序）中止，排在其后的 `/finance/plan` 和 `/user/manage` 未被执行。待 Dash 侧在途改动处理完成后补齐对照例外并重跑 |
