...
## 批量权限组选项筛选（1.14.1，未发布）

- 本地面板版本为 `1.14.1`，修改基线为 `73425d835f6d16746d125b4a88dca9d9c40bf7d4`；当前正式版本仍为下节的 `1.14.0`，尚未创建本次 Tag、Release 或镜像。
...
- 本地验证：管理端 55 项测试通过，包含新增的 10 项权限组菜单测试；PHP `8.4.21`、内存 SQLite 的批量权限组测试 1 项、14 个断言通过，覆盖重复增量操作和未选节点保留。
- 从修改基线的旧补丁升级与全新应用补丁生成的产物一致；重复执行、资源名称与入口引用、JavaScript 和 PHP 语法、发布工作流检查及锚点变化时不改写原文件的验证通过。
