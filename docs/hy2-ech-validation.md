...
本节的握手和转发在 Windows 本机回环地址完成，测试使用临时生成的证书、ECH 密钥和身份。
...
- 本机默认 `CGO_ENABLED=0`，且没有可用的 C 编译器，无法在本机运行 `go test -race`。后续 Node 正式 Linux CI 已完成全量数据竞争检测，661 项测试及子测试全部通过，详见兼容矩阵。
- 本地阶段完成 Linux 两种架构交叉构建；正式 CI 进一步完成 Linux amd64 回环转发及双架构镜像版本检查，arm64 版本检查使用 QEMU。本次没有进行 arm64 实际转发或真实服务器测试。
...
开发构建带有 `vcs.modified=true`，不作为正式发布产物。正式发布从固定 Tag／commit 重新构建，
来源、产物摘要及发布结果见兼容矩阵。先升级 Node，再升级面板并刷新客户端订阅。若回退到 Node `v1.13-yz.21`／面板 `1.11.0`，
