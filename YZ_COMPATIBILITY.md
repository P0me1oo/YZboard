...
- 来源基线为 `1.21.2` / `f49dd94655825c4d8748a98111fa8e76befe778d`，在分支 `feat/device-limit-report` 开发。`server.limit.exceeded` 钩子的事件类型从并发、速率扩展到设备数；设备数事件额外带被拒来源地址，面板只保留合法 IP，去重后最多 5 个。并发和速率事件的字段不变，无数据库迁移。
- 配套 Node `v1.18.0`（未发布，分支 `feat/device-limit-report`）。旧 Node 不上报设备数事件；旧面板收到新 Node 的设备数事件会丢弃该条，其余上报照常处理，因此两边升级不分先后。核心固定依赖和节点配置不变。
- 验证：面板完整 PHP 回归 295 项、3584 个断言通过，其中连接限制专项 13 项覆盖设备数事件的转交、来源地址过滤与截断、负数归零，以及并发事件不带来源字段。PHP 测试通过命令行临时加载本机已有 SQLite 扩展完成；未发布，未进行真实服务器联调。
