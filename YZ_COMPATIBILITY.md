...
- 配套 Node `v1.17.1` 的开发源码已固定核心 `v26.8.1` / `f242ad6931521c1d56245e3566dcf7bc11b9571c`，尚未发布。当前已发布 Node `v1.17.0` 仍固定旧核心，不包含本补丁；仅更新面板不能修复传统客户端连接。
...
- 尚未发布，不提供服务器更新命令；发布时先升级包含兼容补丁的 Node，再更新面板订阅。回滚时需同时考虑核心兼容能力及订阅参数，不能通过删除客户端参数来关闭旧服务端的强制要求。

## 节点列表权限组折叠显示（1.20.5，未发布）
